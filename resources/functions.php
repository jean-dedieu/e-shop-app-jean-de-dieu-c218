...
confirm($query);
    ...
    try {
        ...
        $updateProductWithStripeId = query("UPDATE products SET stripe_product_price = '$stripe_product_price_id', stripe_product_id = '$stripe_product_id' WHERE product_id = $last_id");
        confirm($updateProductWithStripeId);
...
while($row = fetch_array($query)) {
    ...
    $product_price = add2Decimals($row['product_price']);
    $product_total = add2Decimals($row['product_total']);
...
$slide_active = <<<DELIMETER
 ...
 <div class="item active">
    <img class="slide-image" src="../resources/{$slide_image}" alt="{$slide_image}">
</div>
